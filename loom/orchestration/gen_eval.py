"""Generator-Evaluator loop for iterative output refinement.
...
- QualityContract: success criteria negotiated before each sprint
...
class GeneratorEvaluatorLoop:
    ...
    async def run(
        ...
        for sprint_num in range(1, max_sprints + 1):
            ...
            contract = await quality_gate.contract_for(task, iteration=sprint_num)

            # Phase 2: generator produces output
            gen_prompt = self._build_gen_prompt(goal, contract, critique, sprint=sprint_num)
            ...
            quality_result = await quality_gate.evaluate(output, contract)
    ...
    async def _negotiate_contract(self, goal: str, sprint: int) -> QualityContract:
        """Ask the evaluator to generate verifiable success criteria."""
        if self.quality_gate is None:
            raise ValueError("GeneratorEvaluatorLoop requires evaluator or quality_gate")
        return await self.quality_gate.contract_for(
    ...
    def _build_gen_prompt(
        ...
        *,
        ...
        prompt = (
            ...
            f"**Goal:** {goal}\n\n"
            f"**Success criteria (Sprint {sprint}):**\n{criteria_md}\n"
        )
        if critique:
    ...
    def _build_eval_prompt(self, output: str, contract: QualityContract) -> str:
        ...
        return gate.build_evaluation_prompt(output, contract)
