...
class StructureHealthAssessor:
    ...
    def __init__(
        ...
        # Normalize weights
        total = balance_weight + efficiency_weight + performance_weight + utilization_weight
        self.balance_weight /= total
        self.efficiency_weight /= total
        self.performance_weight /= total
        self.utilization_weight /= total

    def assess(self, root: Any, target_config: Any | None = None) -> HealthReport:
        """
        ...
        recommendations = self._generate_recommendations(diagnostics, structure_data, target_config)
    ...
    def _assess_balance(self, data: dict[str, Any], config: Any | None) -> float:
        """Assess structure balance (0-1, higher is better)"""
        score = 1.0

        # Factor 1: Depth variation (penalize unbalanced tree)
        if data["depths"]:
            depth_variance = sum((d - data["avg_depth"]) ** 2 for d in data["depths"]) / len(
                data["depths"]
            )
    ...
    def _assess_utilization(self, data: dict[str, Any]) -> float:
        ...
        low_activity = sum(1 for n in data["nodes"] if 0 < n.metrics.task_count < 3)
    ...
    def _run_diagnostics(self, data: dict[str, Any], config: Any | None) -> list[HealthDiagnostic]:
        ...
        low_perf = [
            n for n in data["nodes"] if n.metrics.task_count > 3 and n.metrics.fitness_score() < 0.4
        ]
        if low_perf:
            diagnostics.append(
                HealthDiagnostic(
        ...
        if data["avg_tokens"] > 6000:
            heavy_nodes = [
                n for n in data["nodes"] if n.metrics.task_count > 0 and n.metrics.avg_tokens > 8000
            ]
            if heavy_nodes:
                diagnostics.append(
                    HealthDiagnostic(
    ...
    def _generate_recommendations(
        self, diagnostics: list[HealthDiagnostic], data: dict[str, Any], _config: Any | None
    ) -> list[str]:
        ...
        for diag in diagnostics:
            if diag.severity > 0.5:
                recommendations.append(f"[{diag.issue.value.upper()}] {diag.recommendation}")
    ...
    def format_report(self, report: HealthReport) -> str:
        ...
        lines.append(
            f"  Balance:      {report.balance_score:.2f} {'▓' * int(report.balance_score * 10)}{'░' * (10 - int(report.balance_score * 10))}"
        )
        lines.append(
            f"  Efficiency:   {report.efficiency_score:.2f} {'▓' * int(report.efficiency_score * 10)}{'░' * (10 - int(report.efficiency_score * 10))}"
        )
        lines.append(
            f"  Performance:  {report.performance_score:.2f} {'▓' * int(report.performance_score * 10)}{'░' * (10 - int(report.performance_score * 10))}"
        )
        lines.append(
            f"  Utilization:  {report.utilization_score:.2f} {'▓' * int(report.utilization_score * 10)}{'░' * (10 - int(report.utilization_score * 10))}"
        )
        lines.append("")
