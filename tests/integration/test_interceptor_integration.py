...
class TestMetricsInterceptor:
    ...
    async def test_metrics_interceptor_collects_stats(self):
        ...
        assert metrics["completed_tasks"] == 2, f"Expected 2 completed tasks, got {metrics['completed_tasks']}"
        assert metrics["failed_tasks"] == 0, f"Expected 0 failed tasks, got {metrics['failed_tasks']}"
