...
import pytest
from loom.skills.activator import match_trigger, match_trigger_async, _semantic_score
...
class TestActivatorMatchAll:
    def test_match_all_true_partial(self):
        skill = Skill(name="s", trigger=SkillTrigger(type="keyword", keywords=["python", "rust"], match_all=True), priority=0.8)
    ...
    def test_custom_evaluator_zero(self):
        skill = Skill(name="c", trigger=SkillTrigger(type="custom", evaluator=lambda t: 0), priority=0.5)
        assert match_trigger(skill, "anything") is None

    def test_custom_evaluator_none(self):
        skill = Skill(name="c", trigger=SkillTrigger(type="custom", evaluator=lambda t: None), priority=0.5)
        assert match_trigger(skill, "anything") is None
...
class TestActivatorAsync:
    ...
    async def test_async_keyword(self):
        skill = Skill(name="k", trigger=SkillTrigger(type="keyword", keywords=["python"]), priority=0.8)
        result = await match_trigger_async(skill, "python help")
        assert result is not None

    async def test_async_keyword_no_match(self):
        skill = Skill(name="k", trigger=SkillTrigger(type="keyword", keywords=["rust"]), priority=0.8)
        result = await match_trigger_async(skill, "python help")
        assert result is None

    async def test_async_semantic_with_embedder(self):
        embedder = MockEmbeddingProvider()
        skill = Skill(name="sem", trigger=SkillTrigger(type="semantic", keywords=["python"], threshold=0.0), priority=0.8)
        result = await match_trigger_async(skill, "python code", embedder=embedder)
        assert result is not None

    async def test_async_semantic_no_match(self):
        embedder = MockEmbeddingProvider()
        skill = Skill(name="sem", trigger=SkillTrigger(type="semantic", keywords=["python"], threshold=1.1), priority=0.8)
