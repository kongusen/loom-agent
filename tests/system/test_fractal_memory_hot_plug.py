...
from loom.memory.types import MemoryUnit, MemoryTier, MemoryType, ContextProjection, MemoryQuery
...
async def test_hot_plug_lifecycle(infrastructure):
    ...
    # 2. Hot-plug Child
    print(f"🔌 Hot-plugging child node...")
...
async def test_memory_sharing_and_isolation(infrastructure):
    ...
    print(f"🔌 Hot-plugging child with shared memory...")
    child_agent = root_agent.add_child(
        role=NodeRole.SPECIALIST,
...
async def test_context_projection_init(infrastructure):
    ...
    print(f"🔍 Verifying projected context in child memory...")
    ...
    assert found_fact
    print(f"✅ Projection Fact found in Child Memory")
