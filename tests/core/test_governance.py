...
class TestToolGovernance:
    ...
    def test_destructive_policy_blocks_tool(self):
        gov = ToolGovernance(GovernanceConfig(allow_destructive=False))
        ok, reason = gov.check_permission(
            "Delete",
...
class TestToolExecutor:
    ...
    async def test_executor_runs_allowed_tool(self):
        ...
        result = await executor.execute(ToolCall(id="1", name="Read", arguments={"text": "hello"}))
    ...
    async def test_executor_blocks_denied_tool(self):
        ...
        governance = ToolGovernance(GovernanceConfig(deny_tools={"Bash"}))
    ...
    async def test_executor_enforces_read_only_mode(self):
        ...
        governance = ToolGovernance(GovernanceConfig(read_only_only=True))
        executor = ToolExecutor(registry, governance)

        result = await executor.execute(ToolCall(id="1", name="Write", arguments={"text": "hello"}))
        assert result.is_error is True
    ...
    async def test_executor_enforces_rate_limit(self):
        registry = ToolRegistry()
        registry.register(make_tool("Read"))
        governance = ToolGovernance(GovernanceConfig(max_calls_per_minute=1))
        executor = ToolExecutor(registry, governance)
