...
class BenchResult:
    ...
    def ops_per_sec(self):
        return 1000 / self.mean if self.mean > 0 else float("inf")
    ...
    for i in range(3):
        ...
        class _P:
            ...
            async def provide(self, q, budget):
                return [
                    ContextFragment(
                        source=self.source, content=f"frag {q}", tokens=10, relevance=0.8
                    )
                ]
...
async def bench_hybrid_retrieval(n=50):
    ...
    docs = [
        Document(id=f"d{i}", content=f"Python programming language tutorial part {i}")
        for i in range(20)
    ]
