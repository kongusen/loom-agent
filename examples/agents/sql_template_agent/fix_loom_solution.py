...
async def main():
    ...
    agent = build_agent(
        ...
        max_iterations=10,
        system_instructions="""你是 SQL 专家。请按以下步骤工作：
1. 使用 schema_lookup 工具查找相关表
2. 使用 doris_select 工具获取样例数据
3. 基于工具结果生成 SQL

重要：在工具调用完成后，必须生成 SQL！不要结束对话！"""
    )
    
    # 3. 测试提示词 - 在每次递归中重复任务目标
    prompt = """
