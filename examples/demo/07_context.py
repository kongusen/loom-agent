...
async def main():
    ...
    await kb.ingest(
        [
            Document(
                id="d1",
                content="Loom 记忆系统分三层：L1 SlidingWindow 滑动窗口保存近期对话，L2 WorkingMemory 按重要性存储关键信息，L3 PersistentStore 持久化长期记忆",
            ),
            Document(
                id="d2",
                content="Loom ContextOrchestrator 使用 EMA 自适应算法动态分配 token 预算，高相关源获得更多预算",
            ),
        ]
    )
